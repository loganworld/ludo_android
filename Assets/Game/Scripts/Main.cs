...
public class Main : MonoBehaviour
{
    ...
    public void Button()
    {
        ...
        else if (EventSystem.current.currentSelectedGameObject.name == "Play")
        {
            ...
            //if (Int32.Parse(betAmount.text) < Global.balance)
            //{
            //    betAmount.text = Global.balance.ToString();
            //}
            if (Int32.Parse(betAmount.text) < Global.minAmount)
            {
                betAmount.text = "0";
            }

            Global.betAmount = betAmount.text;
